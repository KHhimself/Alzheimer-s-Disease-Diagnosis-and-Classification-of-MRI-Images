...
class ResNet18Custom(nn.Module):
    def __init__(self, num_classes=4):
        ...
        self.base_model.conv1 = nn.Conv2d(
            ...
            out_channels=64,
            kernel_size=3,  # Change 7x7 to 3x3
            stride=1,       # Avoid excessive downsampling
            padding=1,      # Match 3x3 convolution
            bias=False
        )
        self.base_model.fc = nn.Sequential(
            nn.Linear(self.base_model.fc.in_features, 64),
            nn.ReLU(),
            nn.BatchNorm1d(64),
            nn.Dropout(0.5),
...
data_path = '.'  # Replace with actual dataset path
...
processed_images = processed_images.reshape(-1, 1, 45, 45)
...
X_train_resampled = X_train_resampled.reshape(-1, 1, 45, 45)
